...
void *blur_x(void *arguments) {
    ...
    for (r = 0; r < args->rows; r++) {
        for (c = args->start; c < args->end; c++) {
            dot = sum = 0.0;
            for (cc = -args->center; cc <= args->center; cc++) {
                if ((c + cc) >= 0 && (c + cc) < args->cols) {
                    dot += (float)args->image[r * args->cols + (c + cc)] * args->kernel[args->center + cc];
                    sum += args->kernel[args->center + cc];
                }
            }
            args->tempim[r * args->cols + c] = dot / sum;
        }
    }
    return NULL;
}
...
void *blur_y(void *arguments) {
    ...
    for (c = 0; c < args->cols; c++) {
        for (r = args->start; r < args->end; r++) {
            dot = sum = 0.0;
            for (rr = -args->center; rr <= args->center; rr++) {
                if ((r + rr) >= 0 && (r + rr) < args->rows) {
                    dot += args->tempim[(r + rr) * args->cols + c] * args->kernel[args->center + rr];
                    sum += args->kernel[args->center + rr];
                }
            }
            (*args->smoothedim)[r * args->cols + c] = (short int)(dot * BOOSTBLURFACTOR / sum + 0.5);
        }
    }
    return NULL;
}
...
void gaussian_smooth(unsigned char *image, int rows, int cols, float sigma, short int **smoothedim) {
    ...
    float *tempim, *kernel;
    pthread_t thread_x[N_T], thread_y[N_T];
    struct thread_args args_x[N_T], args_y[N_T];
    ...
    *smoothedim = (short int *)calloc(rows * cols, sizeof(short int));
    ...
    for (int i = 0; i < N_T; i++) {
        args_x[i] = (struct thread_args){image, rows, cols, col_per_t * i, col_per_t * (i + 1), center, kernel, tempim, smoothedim};
        pthread_create(&thread_x[i], NULL, blur_x, &args_x[i]);
    }
    for (int i = 0; i < N_T; i++) {
    ...
    for (int i = 0; i < N_T; i++) {
        args_y[i] = (struct thread_args){image, rows, cols, row_per_t * i, row_per_t * (i + 1), center, kernel, tempim, smoothedim};
        pthread_create(&thread_y[i], NULL, blur_y, &args_y[i]);
    }
    for (int i = 0; i < N_T; i++) {
